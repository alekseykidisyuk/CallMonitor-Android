...
public class MockAdb {
    public static int Main(string[] args) {
        ...
        if (a.Contains("dumpsys package")) { Console.WriteLine("versionCode=220020 minSdk=30 targetSdk=36"); return 0; }
        return 3;
    }
}
'@
...
$Apk = Join-Path $TestRoot 'CallMonitor-Android-build20.apk'
...
if ($LASTEXITCODE -ne 0 -or !(($Result -join "`n") -match 'PASS: CallMonitor build 20 installed')) { throw ($Result | Out-String) }
